...
class CMesh {
public:
	int shapeChangesToMesh( CVector<double> shapeParams );
	int shapeChangesToMesh( CVector<float> shapeParams );
	int updateJntPos();
	static void readShapeSpaceEigens( std::string fileName, int nos = NO_OF_EIGENVECTORS);
	// constructor
	inline CMesh();
	...
	inline CVector<float> GetPoint(int i);
...
CVector<float> CMesh::GetPoint( int i )
{
	CVector<float> tmp; tmp.setSize(3);
	float x,y,z;
	GetPoint(i,x,y,z);

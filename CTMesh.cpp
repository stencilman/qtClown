...
CMesh::CMesh(int aPatches, int aPoints) {
	//mPoints = new CVector<float>[aPoints];
	//mPatch = new CVector<int>[aPatches];
	...
	//      mPointsS=new CVector<float>[aPoints];
	//   for(int i=0;i<aPoints;i++)
	//      {
...
bool CMesh::readModelHybOpt(const char* aFilename, bool smooth) {
	...
	//mPoints = new CVector<float>[mNumPoints]; 
	//mPatch = new CVector<int>[mNumPatch];
...
void CMesh::writeAdaptModel(const char* aFilename, const CMesh* adM) {
	...
	for (int i = 0; i < mNumPoints; i++) {
		...
		if(!IsCovered( GetBodyPart(int(mPoints[i][3])) ) ) {
			...
			//       aStream << adM->mPoints[i][0] << " ";   
			//       aStream << adM->mPoints[i][1] << " ";
			//       aStream << adM->mPoints[i][2] << std::endl;
		}
#endif    
...
bool CMesh::adaptOFF(const char* aFilename, float lambda) {
	...
	if(aStream.is_open()) {
		...
		for (int i = 0; i < mNumPoints; i++) {
			...
			// 	  float tmp;
			// 	  aStream >> tmp;
			// 	  mPoints[i][j] *= (1-mPoints[i][4]);
			// 	  mPoints[i][j] += mPoints[i][4]*tmp;
			// 	}
			//       }
		}
		cout << "ok" << endl;
...
void CMesh::writeSkel(const char* aFilename) {
	...
	//-1.2305|-1137.47|30.38|0|
	//CVector<float> trans(3);
	//trans(0) = -793.797+1.2305; trans(1) = -1429.45+1137.47; trans(2) = -258.605-30.38; 
	//|||1|
	//cout << mCenter;
...
void CMesh::rigidMotion(CVector<CMatrix<float> >& M,CVector<float>& X, bool smooth, bool force) {
	...
	// cout << "RBM Points " << mNumPoints << endl;
	// cout << M(0) << endl;
...
void CMesh::operator=(const CMesh& aMesh) {
	...
	//cout << "mJointNumber " <<  mJointNumber  << endl; 
	...
	//   for(int i=0;i<mNumPoints;i++)
	//     {
	//       mPoints[i]=aMesh.mPoints[i];
	//       //mPointsS[i]=aMesh.mPoints[i];
	//     }
	//   for(int i=0;i<mNumPatch;i++)
	//     {
	//       mPatch[i]=aMesh.mPatch[i];;
	//     }
	...
	//mHistory = aMesh.mHistory;
	//mAppearanceField = aMesh.mAppearanceField;

	//cout << "Copy constructor ende " << endl;

}

// newFrame
// void CMesh::newFrame() {
//   for (int t = mHistory.size()-2; t >= 0; t--)
//     mHistory(t+1) = mHistory(t);
//   mHistory(0) = mCurrentMotion;
// }
...
void CMesh::projectToImage(CMatrix<float>& aImage, CMatrix<float>& P, int aLineValue)
{
	...
	for(int i=0;i<Size;i++)
	{
		...
		if(det<0.001)
		{
			...
			j=0;
			//(*this).projectPoint(P, mPoints[ I[j] ](0),mPoints[ I[j] ](1),mPoints[ I[j] ](2),ax,ay   );
			(*this).projectPoint(P, mPoints[ I[j] ](0),mPoints[ I[j] ](1),mPoints[ I[j] ](2),bx,by);
			if (ax >= 0 && ay >= 0 && ax < aImage.xSize() && ay < aImage.ySize())
				aImage.drawLine(ax,ay,bx,by,aLineValue);
		}
...
void CMesh::projectToImageJ(CMatrix<float>& aImage, CMatrix<float>& P, int aLineValue, int aJoint, int xoffset, int yoffset)
{
	...
	for(int i=0;i<Size;i++)
	{
		...
		// if(det<-0.001)
		{
			//if(fabs((*this).GetJointID(I[0])-aJoint)<=1)
...
void CMesh::readShapeSpaceEigens( std::string fileName, int nos )
{
	string path = fileName;		
	ifstream loadFile;
	loadFile.open(path.c_str());
	//reading only first four columns here
	char fullLine[10000], c[NO_OF_EIGENVECTORS][500];
	eigenVectors.resize(NO_OF_EIGENVECTORS);
	for (unsigned int i0 = 0; i0 < NO_OF_EIGENVECTORS; i0++)
	{
	...
	while(!loadFile.getline(fullLine, 10000).eof())
	{
		...
		//////////////////////////////////////////////////////////////////////////
		//sscanf(fullLine,"%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
		//	&c[0], &c[1], &c[2], &c[3], 
		...
		sscanf(fullLine,"%s%s%s%s%s%*s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
			...
			&c[10], &c[11], &c[12], &c[13], 
			&c[14], &c[15], &c[16], &c[17],&c[18], &c[19]);	
		/*sscanf(fullLine,"%s%s%s%s%s",
			&c[0], &c[1], &c[2], &c[3], &c[4]);	*/
		/*sscanf(fullLine,"%s%s%s%s%s%s%s%s%s%s",&c[0], &c[1], &c[2], &c[3], 
			&c[4], &c[5], &c[6], &c[7],&c[8], &c[9]);*/
		//////////////////////////////////////////////////////////////////////////
		for (unsigned int i0 = 0; i0 < NO_OF_EIGENVECTORS; i0++)
		{	
...
{
	CVector<double> ishapeParams; ishapeParams.setSize(shapeParams.size());
	for (unsigned int i0 = 0; i0 < shapeParams.size(); i0++)
		ishapeParams(i0) = shapeParams(i0);

	shapeChangesToMesh(ishapeParams);
	return true;
}
int CMesh::shapeChangesToMesh( CVector<double> shapeParams )
{
	//cout << "\nChanging the mesh based on the shape params.. ";	
	//mPoints[lot of rows][0 1 2]	
	...
	for(unsigned int i1 = 0; i1 < mPoints.size(); i1++)
	{
		for(unsigned int i2 = 0; i2 < 3; i2++)
		{
			...
			{					
				...
				(mPoints[i1])[i2] += shapeParams[i0]*1000.0*eigenVectors[i0](i1,i2);									
			}
		}
	}	
	//cout<<"Done!";
...
int CMesh::updateJntPos()
{
	...
	//////////////////////////////////////////////////////////////////////////
	//Now update the joints as per our old map
	//here we find joint no 2 (jid = 3) which is the root
	...
	//////////////////////////////////////////////////////////////////////////
	// Now, modify a copy of parent map to the old one (nils one)
	...
	//////////////////////////////////////////////////////////////////////////
	// Copy it back to the joints
	//////////////////////////////////////////////////////////////////////////
	
	//////////////////////////////////////////////////////////////////////////
	//This stuff is finding the axis for the elbow
	//////////////////////////////////////////////////////////////////////////
	//CVector<double> axisLeftArm, axisRightArm;
	//findNewAxesOrient(axisLeftArm, axisRightArm, newJntPos);
	//////////////////////////////////////////////////////////////////////////	
	...
	for(unsigned int i0 = 0; i0 < mJointNumber; i0++)
	{
		...
		/*if(jId == 8)
			for(unsigned int j0 = 0; j0 < 3; j0++)
				aDir(j0) = (float)axisRightArm(j0);
		else if(jId == 12)
			for(unsigned int j0 = 0; j0 < 3; j0++)
				aDir(j0) = (float)axisLeftArm(j0);*/
	...
	//writeMeshDat("H:\\unix-home\\tools\\tmpMesh.dat");
	//printf("\n------------------- Joint Positions ---------------------");
